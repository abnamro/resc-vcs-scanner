...
def test_write_correct_repository(post):
    ...
    expected_result = RepositoryRead(
        id_=1,
...
def test_write_scan(post):
    ...
    result = RESTAPIWriter(rws_url=url).write_scan(
...
@patch("vcs_scanner.output_modules.rws_api_writer.RESTAPIWriter.download_rule_pack")
def test_check_active_rule_pack_version_when_rule_pack_version_not_provided(
    download_rule_pack,
):
    url = "https://nonexistingwebsite.com"
...
def test_get_active_rule_pack_version_successful(get):
    url = "https://nonexistingwebsite.com"
    expected_result = (
        '{"data": [ {"version": "1.0.0", "active": "true", "global_allow_list": 1}], "total": 1, '
        '"limit": 100, "skip": 0} '
    )
    get.return_value.status_code = 200
