...
class STDOUTWriter(OutputModule):
    def __init__(
        ...
        self.ignore_findings_providers: IgnoredListProvider = IgnoredListProvider(
            ignore_findings_path
        )
    ...
    @staticmethod
    def _determine_finding_action(
        finding: FindingCreate, rule_tags: dict = None
    ) -> FindingAction:
        """
    ...
    def _determine_if_ignored(
        ...
        key: str = (
            finding_path + "|" + finding.rule_name + "|" + str(finding.line_number)
        )
    ...
    def write_findings(
        self, scan_id: int, repository_id: int, scan_findings: List[FindingCreate]
    ):
        """
        ...
        rule_comments = (
            get_rule_comment(self.toml_rule_file_path)
            if self.toml_rule_file_path
            else None
        )
        ...
        for finding in scan_findings:
            ...
            if should_process:
                ...
                if finding_action == FindingAction.BLOCK:
                    finding_action_value = colored(
                        finding_action.value, "red", attrs=["bold"]
                    )
                    block_count += 1
                elif finding_action in [FindingAction.WARN, FindingAction.IGNORED]:
                    finding_action_value = colored(
                        finding_action.value, "light_red", attrs=["bold"]
                    )
                    warn_count += 1
                elif finding_action == FindingAction.INFO:
                    finding_action_value = colored(
                        finding_action.value, "light_yellow", attrs=["bold"]
                    )
                    info_count += 1
                else:
                ...
                comment = (
                    rule_comments.get(finding.rule_name, "") if rule_comments else ""
                )
        ...
        if exit_code == self.exit_code_success:
            logger.info(
                f"Findings threshold check results: {colored('PASS', 'light_green', attrs=['bold'])}"
            )
        elif exit_code == self.exit_code_block:
            logger.info(
                f"Findings threshold check results: {colored('FAIL', 'red', attrs=['bold'])}"
            )
            logger.info(
                colored(
    ...
    def write_scan(
        ...
        logger.info(
            f"Running {scan_type_to_run} scan on repository {repository.repository_url}"
        )
        return ScanRead(
